...
<div class="container">
    ...
    {{-- <div style="text-align: right;margin-right:5px;right: 0;top: 90px;">
        <span style="font-size:6x;">{{__('Amounts expressed in')}} {{auth()?->user()?->company?->platformCountry?->currency}}</span>
    </div>
        ...
        <table width="100%" border="0">
            ...
            @foreach ($invoice->details->whereNull('canceled_at') as $detail)
                <tr class="color1">
                    ...
                    <td>
                        {{ $detail->price }} {{ auth()?->user()?->company?->platformCountry?->currency }}
                    </td>
                    <td>
                        <strong>
                            {{ ($detail->price*$detail->quantity) }} {{ auth()?->user()?->company?->platformCountry?->currency }}
                        </strong>
                    </td>
                </tr>
            @endforeach
        ...
        <div class="section4-child">
            <div class="section4-line">
                ...
                <div class="value float-right">
                    {{ isset($invoice->taxes['taxes']) ? $invoice->taxes['preTax'] : $invoice->total }}  {{ auth()?->user()?->company?->platformCountry?->currency }}
                </div>
            </div>
            ...
            @if (isset($invoice->taxes['taxes']))
                @foreach ($invoice->taxes['taxes'] as $key => $tax)
                    <div class="section4-line">
                        ...
                        <div class="value float-right">
                            {{ $tax }} {{ auth()?->user()?->company?->platformCountry?->currency }}
                        </div>
                    </div>
                @endforeach
            @endif
            ...
            <div class="section4-line">
                ...
                <div class="value float-right">
                    {{ $invoice->total }} {{ auth()?->user()?->company?->platformCountry?->currency }}
                </div>
            </div>
        </div>
    </div>
